...
class ActionBacklog extends Action
{
    ...
    protected $titles = array(self::CREATE_BACKLOG => "New backlog task created", self::EDIT_BACKLOG => "Backlog task edited", 
            self::FINALIZED_BACKLOG => "Backlog task finalized", self::NEW_ISSUE=>"A new issue added to backlog task");
    protected $description = array(self::CREATE_BACKLOG => "A new backlog task has been created in project %p% with name %b%",
    ...
    /**
     * @ORM\ManyToOne(targetEntity="EasyScrumREST\ProjectBundle\Entity\Backlog", inversedBy="actions")
     */
    private $backlog;
