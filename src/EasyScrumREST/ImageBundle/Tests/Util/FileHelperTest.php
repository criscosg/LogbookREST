...
class FileHelperTest extends CustomTestCase
{
    ...
    public function testResizeAndSaveImageWithCropAndConvert()
    {
        ...
        $image='01.jpg';
        $sufix='test';
        $directory = $this->container->getParameter('kernel.root_dir').'/../src/EasyScrumREST/ImageBundle/Tests/File';
        FileHelper::resizeAndSaveImage($image, $width, $height, $directory, $sufix);
        $fileNameThumb=$this->container->getParameter('kernel.root_dir').'/../src/EasyScrumREST/ImageBundle/Tests/File/01-test.jpg';
        list($thumbWidth, $thumbHeight) = getimagesize($fileNameThumb);
        $this->assertTrue($thumbWidth == 150 && $thumbHeight == 150);

        $fileNameThumbconvert=$this->container->getParameter('kernel.root_dir').'/../src/EasyScrumREST/ImageBundle/Tests/File/01-test-convert.jpg';
